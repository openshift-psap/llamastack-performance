...
# Sequential vLLM-only Concurrency Testing Script (Template-Based)
# This script uses separate YAML template files for job definitions
# Each test gets fresh vLLM deployments and benchmarks vLLM directly (no LlamaStack)
#
# Usage: ./run-sequential-vllm-with-templates.sh <output-directory>
# Example: ./run-sequential-vllm-with-templates.sh ./results/vllm-baseline-test
...
if [ -z "$1" ]; then
    ...
    echo "Example: $0 ./results/vllm-baseline-test"
    exit 1
fi
...
GUIDELLM_TEMPLATE="$TEMPLATE_DIR/guidellm-vllm-job-template.yaml"
...
substitute_template() {
    ...
    envsubst '$CONCURRENCY,$VLLM_SERVICE_NAME,$NAMESPACE_BENCH' < "$template_file" > "$output_file"
}
...
save_logs() {
    local concurrency="$1"
    local job_name="guidellm-vllm-concurrency-$concurrency"
...
# Main execution
main() {
    log "Starting Sequential vLLM-only Concurrency Testing (Direct vLLM Benchmarking)"
    log "Results will be saved to: $BASE_DIR"
    
    # Check if template files exist
    if [ ! -f "$VLLM_TEMPLATE" ] || [ ! -f "$GUIDELLM_TEMPLATE" ]; then
    ...
    for concurrency in "${CONCURRENCIES[@]}"; do
        ...
        if wait_for_job_completion "guidellm-vllm-concurrency-$concurrency"; then
    ...
    success "All vLLM-only concurrency tests completed!"
