...
class ResponsesDirectUser(HttpUser):
    """Responses API direct to vLLM — baseline for measuring LlamaStack overhead.

    Same payload as ResponsesSimpleUser but sent directly to vLLM's /v1/responses
    endpoint, bypassing LlamaStack entirely. 

    When INPUT_TOKENS > 0, uses synthetic prompts from synthetic_prompts.jsonl.
    When OUTPUT_TOKENS > 0, sets max_output_tokens.
    """
    wait_time = constant(0)
    abstract = True
    _prompts = None
    ...
    def on_start(self):
        self.model = os.environ.get("MODEL", "qwen3-vl-30b-a3b-instruct")
        self.input_tokens = int(os.environ.get("INPUT_TOKENS", "0"))
        self.output_tokens = int(os.environ.get("OUTPUT_TOKENS", "0"))
        self.default_prompt = os.environ.get("PROMPT", "What is the capital of France?")

        if self.input_tokens > 0 and ResponsesDirectUser._prompts is None:
            ResponsesDirectUser._prompts = _load_prompts()
        print(f"[ResponsesDirectUser] on_start: input_tokens={self.input_tokens}, output_tokens={self.output_tokens}, prompts_available={len(ResponsesDirectUser._prompts) if ResponsesDirectUser._prompts else 0}", file=sys.stderr, flush=True)

    @task
    def call_responses_direct(self):
        if ResponsesDirectUser._prompts:
            with _user_counter_lock:
                global _user_counter
                idx = _user_counter
                _user_counter += 1
            prompt = ResponsesDirectUser._prompts[idx % len(ResponsesDirectUser._prompts)]
        else:
            prompt = self.default_prompt
        payload = {
            "model": self.model,
